...
public class OriginalRepo(ArchiveDbContext context) : IOriginalRepo
{
    ...
    public async Task<Result<Original>> GetOriginalAsync(int id)
    {
        var original = await _context.Originals
            ...
            .AsNoTracking()
            .FirstOrDefaultAsync(x => x.Id == id);
        return original != null ? Result<Original>.Success(original) : Result<Original>.Fail("Оригинал не найден");
    }
    public async Task<Result<List<Original>>> GetOriginalList()
        => Result<List<Original>>.Success(await _context.Originals.AsNoTracking().Include(x => x.Document).OrderBy(x => x.InventoryNumber).ToListAsync());
    public async Task<Result<List<Original>>> GetOriginalsByDocument(int docunentId)
        => Result<List<Original>>.Success(await _context.Originals.AsNoTracking().Where(x => x.DocumentId == docunentId).ToListAsync());
    public async Task<Result<List<Original>>> GetOriginalsByCompany(int companyId)
        => Result<List<Original>>.Success(await _context.Originals.AsNoTracking().Where(x => x.CompanyId == companyId).ToListAsync());
    public async Task<Result<List<Original>>> GetOriginalsByApplicability(int applicabilityId)
        => Result<List<Original>>.Success(await _context.Originals.AsNoTracking().Where(x => x.Applicabilities
        .Contains(_context.Applicabilities.First(y => y.Id == applicabilityId))).ToListAsync());

    public async Task<Result<int>> GetLastInventoryNumberAsync() => Result<int>.Success(await _context.Originals.MaxAsync(y => y.InventoryNumber));
    public async Task<Result<Nothing>> CheckInventoryNumberAsync(int inventoryNumber)
    {
        bool result = await _context.Originals.AnyAsync(x => x.InventoryNumber == inventoryNumber);
        return result ? Result<Nothing>.Fail("Инвернтарный номер занят") : Result<Nothing>.Success();
    }
    ...
    {
        //птом ещё раз попробовать чтобы дата изменений создавалась в контексте автоматически
        original.CreatedDate = DateTime.Now;
        await _context.Originals.AddAsync(original);
        await _context.SaveChangesAsync();

        return original.Id > 0 ? Result<int>.Success(original.Id) : Result<int>.Fail("Не удалось создать оригинал");
    }
    private async Task<Result<int>> UpdateOriginal(Original original)
    {
        ...
        if (dbOriginal == null)
        {
            return Result<int>.Fail("Оригинал не найден при попытке редактировании");
        }
    ...
    public async Task<Result<Nothing>> DeleteOriginal(int id)
    {
        ...
        if (original == null)
        {
            return Result<Nothing>.Fail("Оригинал не найден при попытке удаления");
        }
        original.IsDeleted = true;
        await _context.SaveChangesAsync();
    ...
    public async Task<Result<Nothing>> UpdateOriginalApplicabilities(int id, List<int> applicabilityIds)
    {
        ...
        return Result<Nothing>.Fail("Не удалость изменить применимость оригинала");
    }
}
