...
public class ArchiveDbContext : DbContext
{
    ...
    private void SetTimestamps()
    {
        ...
        {
            if (entry.Entity is FullAuditableModel model)
            {
                entry.Property(nameof(FullAuditableModel.LastModifiedDate)).CurrentValue = DateTime.Now;

                if (entry.State == EntityState.Added)
                {
                    entry.Property(nameof(FullAuditableModel.CreatedDate)).CurrentValue = DateTime.Now;
                }
            }
        }
    }
